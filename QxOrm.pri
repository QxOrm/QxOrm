...
QMAKE_CFLAGS_RELEASE += -Zi -Oy
QMAKE_CXXFLAGS_RELEASE += -Zi -Oy
} # CONFIG(debug, debug|release)
win32-g++: QMAKE_LFLAGS += -Wl,-export-all-symbols -Wl,-enable-auto-import
} # win32
unix: QMAKE_CXXFLAGS += -Wno-switch -Wno-reorder -Wno-parentheses -Wno-sign-compare
...
#############################################################
# Qt Widgets Module Dependency - Add Drag And Drop and Undo #
#############################################################

# By default, QxOrm library doesn't depend on Qt Widgets shared library
# Enabling Qt Widgets allows the usage of Undo for IxModel's functionality through QUndoStack and DragNDrop
# Note: This is still experimental and under development by The Jimmytaker. Refer to IxDnDModel.

DEFINES += _QX_ENABLE_QT_WIDGETS

contains(DEFINES, _QX_ENABLE_QT_WIDGETS) {
QT += gui widgets
} # contains(DEFINES, _QX_ENABLE_QT_WIDGETS)
