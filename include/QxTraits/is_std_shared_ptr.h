...
private:
   ...
   template <typename V> static char isSmartPtr(std::shared_ptr<V>);
   static int isSmartPtr(...);
   static T t;

public:

   enum { value = (sizeof(qx::trait::is_std_shared_ptr<T>::isSmartPtr(t)) == sizeof(char)) };

   typedef typename boost::mpl::if_c<qx::trait::is_std_shared_ptr<T>::value, 
                                     boost::mpl::true_, 
